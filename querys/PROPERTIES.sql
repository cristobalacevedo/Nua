SELECT CONCAT(pe.name," ", pe.surname) AS Dueño,
CASE 
...
---JOIN house h ON h.property_id = p.id
---JOIN flat f ON f.property_id = p.id
---JOIN storage s ON s.property_id = p.id
---JOIN land l ON l.property_id = p.id
---JOIN office o ON o.property_id = p.id
--- FILTROS:
--- WHERE pp.name = "Flat"